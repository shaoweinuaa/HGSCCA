function  [opt_c,opt_g,len_E,len_R,len_M,X1,Y1,ind_Epi,ind_RNA,ind_miRNA,accuracy,spec,sens,PPV,NPV,AUC]=HGSCCA(train_epi,test_epi,train_miRNA,test_miRNA,train_RNA,test_RNA,train_label,test_label,opts,k)
...
for i=-10:1:10
    for j=-10:1:10
     ...
     parameter1=['-t ' num2str(2) ,' -c ',num2str(2^i), ' -g ',num2str(2^j)];

     model=svmtrain(double(train_label),double(s_train),parameter1);
...
% parameter=['-t ' num2str(2) ,' -c ',num2str(opt_c),' -g ',num2str(opt_g)];
if opt_c~=0
 parameter=['-t 2',' -c ',num2str(opt_c), ' -g ',num2str(opt_g)];
